...
class MarkerSeekNode(Node):
    ...
    def __init__(self) -> None:
        super().__init__('curbie_smart_bin_vision')
        ...
        self.declare_parameter('image_topic', '/curbie/front_camera/image_raw')
        self.declare_parameter('camera_info_topic', '/curbie/front_camera/camera_info')
        self.declare_parameter('rear_image_topic', '/curbie/rear_camera/image_raw')
        self.declare_parameter('rear_camera_info_topic', '/curbie/rear_camera/camera_info')
        self.declare_parameter('cmd_vel_topic', 'cmd_vel')
        self.declare_parameter('pickup_camera_name', 'rear')
        self.declare_parameter('dropoff_camera_name', 'front')
        self.declare_parameter('manual_camera_name', 'rear')
        self.declare_parameter('manual_use_rear_when_hooked', False)
        self.declare_parameter('hook_status_topic', '/hook/status')
        self.declare_parameter('aruco_dictionary', 'DICT_4X4_50')
        self.declare_parameter('marker_size_m', 0.10)
        self.declare_parameter('stop_distance_m', 0.15)
        ...
        self.declare_parameter('align_search_spin_speed_rad_s', 0.25)
        self.declare_parameter('align_distance_tolerance_m', 0.05)
        self.declare_parameter('align_lateral_tolerance_m', 0.05)
        self.declare_parameter('align_yaw_tolerance_rad', 0.22)
        self.declare_parameter('align_success_hold_steps', 4)
        self.declare_parameter('align_lost_scan_timeout_s', 3.0)
        self.declare_parameter('align_max_lost_retries', 10)
        self.declare_parameter('align_max_reverse_speed_m_s', 0.05)
        self.declare_parameter('align_max_angular_speed_rad_s', 0.40)
        self.declare_parameter('align_use_marker_yaw', False)
        self.declare_parameter('pickup_never_give_up', True)
        self.declare_parameter('cmd_smoothing_enabled', True)
        self.declare_parameter('cmd_linear_accel_limit_m_s2', 0.25)
        self.declare_parameter('cmd_linear_decel_limit_m_s2', 0.70)
        self.declare_parameter('cmd_angular_accel_limit_rad_s2', 1.20)
        self.declare_parameter('cmd_angular_decel_limit_rad_s2', 2.80)
        ...
        self._cmd_smoothing_enabled = bool(
            self.get_parameter('cmd_smoothing_enabled').value
        )
        self._cmd_linear_accel_limit_m_s2 = float(
            self.get_parameter('cmd_linear_accel_limit_m_s2').value
        )
        self._cmd_linear_decel_limit_m_s2 = float(
            self.get_parameter('cmd_linear_decel_limit_m_s2').value
        )
        self._cmd_angular_accel_limit_rad_s2 = float(
            self.get_parameter('cmd_angular_accel_limit_rad_s2').value
        )
        self._cmd_angular_decel_limit_rad_s2 = float(
        ...
        self._align_defaults = {
            ...
            'use_marker_yaw': bool(
                self.get_parameter('align_use_marker_yaw').value
            ),
            'pickup_never_give_up': bool(
                self.get_parameter('pickup_never_give_up').value
            ),
        }
        ...
        self._cmd_linear_applied = 0.0
        self._cmd_angular_applied = 0.0
        self._cmd_last_publish_s = time.monotonic()

        self.cmd_vel_pub = self.create_publisher(TwistStamped, self.cmd_vel_topic, 10)
        self.event_pub = self.create_publisher(String, '/curbie/seek/events', 10)
        ...
        if self.autostart:
            ...
            self.get_logger().info(
                'curbie_smart_bin_vision ready. Call /curbie/seek/start or use /curbie/align_to_marker.'
            )
    ...
    def _image_to_gray(self, msg: Image) -> Optional[np.ndarray]:
        ...
        if encoding in {'yuyv', 'yuy2', 'yuv422', 'yuv422_yuy2'}:
            ...
            if step < line_width:
                return None
            yuyv = raw.reshape((height, step))[:, :line_width].reshape((height, width, 2))
            gray = cv2.cvtColor(yuyv, cv2.COLOR_YUV2GRAY_YUY2)
            return np.ascontiguousarray(gray, dtype=np.uint8)
        return None
    ...
    def _prune_measurements_for_camera(self, camera_name: str, now_s: float) -> None:
        with self._measurements_lock:
            previous = self._latest_measurements.get(camera_name, {})
            if not previous:
                return
            retained = {
                marker_id: measurement
                for marker_id, measurement in previous.items()
                if (now_s - measurement.stamp_s) <= self.measurement_retain_s
            }
            self._latest_measurements[camera_name] = retained
    ...
    def _image_callback(self, msg: Image, camera_name: str) -> None:
        ...
        if frame is None:
            self._prune_measurements_for_camera(camera_name, time.monotonic())
            return
        ...
        with self._measurements_lock:
            previous = self._latest_measurements.get(camera_name, {})
            merged: Dict[int, AlignMeasurement] = {
                marker_id: measurement
                for marker_id, measurement in previous.items()
                if (now_s - measurement.stamp_s) <= self.measurement_retain_s
            }
            merged.update(observations)
            self._latest_measurements[camera_name] = merged
    ...
    def _publish_cmd(
        ...
        *,
        ...
        with self._cmd_lock:
            if immediate:
                self._cmd_linear_applied = float(linear_x)
                self._cmd_angular_applied = float(angular_z)
                self._cmd_last_publish_s = time.monotonic()
            elif self._cmd_smoothing_enabled:
                now_s = time.monotonic()
                dt = max(1e-3, min(0.5, now_s - self._cmd_last_publish_s))
                self._cmd_last_publish_s = now_s
                self._cmd_linear_applied = self._slew_value(
                    current=self._cmd_linear_applied,
                    target=float(linear_x),
                    accel_limit=self._cmd_linear_accel_limit_m_s2,
                    decel_limit=self._cmd_linear_decel_limit_m_s2,
                    dt=dt,
                )
                self._cmd_angular_applied = self._slew_value(
                    current=self._cmd_angular_applied,
                    target=float(angular_z),
                    accel_limit=self._cmd_angular_accel_limit_rad_s2,
                    decel_limit=self._cmd_angular_decel_limit_rad_s2,
                    dt=dt,
                )
            else:
                self._cmd_linear_applied = float(linear_x)
        ...
        msg.twist.linear.x = float(self._cmd_linear_applied)
        msg.twist.angular.z = float(self._cmd_angular_applied)
        try:
    ...
    def _build_alignment_status(
        ...
        *,
        ...
        marker_visible = bool(
            ...
            and (now_s - measurement.stamp_s) <= core.lost_marker_timeout_s
        )
        status = AlignmentStatus()
        status.surface_detected = marker_visible
        ...
        status.distance_to_surface = float(
            measurement.z_m if marker_visible and measurement is not None else 0.0
        )
    ...
    def _execute_align_callback(self, goal_handle):
        ...
        goal_mode = str(goal.mode).strip().lower()
        try:
        ...
        never_give_up = bool(
            goal_mode == 'pickup' and self._align_defaults['pickup_never_give_up']
        )
        align_timeout_s = float(goal.timeout_s)
        align_max_lost_retries = int(self._align_defaults['max_lost_retries'])
        align_lost_marker_timeout_s = float(
            self.get_parameter('lost_marker_timeout_s').value
        )
        if never_give_up:
            # Pickup seek should keep scanning/reacquiring until marker lock.
            align_timeout_s = max(align_timeout_s, 24.0 * 60.0 * 60.0)
            align_max_lost_retries = max(
                align_max_lost_retries,
                1_000_000,
            )
            align_lost_marker_timeout_s = min(align_lost_marker_timeout_s, 0.5)
        controller = AlignToMarkerCore(
            ...
            lost_marker_timeout_s=align_lost_marker_timeout_s,
            ...
            max_lost_retries=align_max_lost_retries,
            ...
            use_marker_yaw=self._align_defaults['use_marker_yaw'],
        )
        ...
        try:
            while rclpy.ok():
                ...
                if obstacle_active:
                    ...
                    feedback.alignment = self._build_alignment_status(
                        controller,
                        measurement,
                        now_s=now_s,
                    )
                ...
                if controller.state == AlignState.FAILED and never_give_up:
                    self.get_logger().info(
                        f'Pickup seek for marker {goal.target_marker_id} reached a '
                        'retry boundary; alternating scan direction and continuing search.'
                    )
                    command = controller.restart_search(now_s=now_s, flip_direction=True)
                self._publish_cmd(command.linear_x, command.angular_z)
                feedback = AlignToMarker.Feedback()
                fresh_measurement = measurement
                if controller.state in {AlignState.SEARCHING, AlignState.LOST_SCAN}:
